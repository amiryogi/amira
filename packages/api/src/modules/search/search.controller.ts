...
export class SearchController {
  static search = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const result = await searchService.search(req.query as unknown as SearchQueryDTO);
  ...
  static suggest = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const suggestions = await searchService.suggest(req.query as unknown as SuggestQueryDTO);
